(() => {
  ...
  const stepSliderEl = document.getElementById("quick-step-slider");
  const playBtn = document.getElementById("quick-play");
  const stopBtn = document.getElementById("quick-stop");
  const prevBtn = document.getElementById("quick-prev");
  const nextBtn = document.getElementById("quick-next");
  const firstBtn = document.getElementById("quick-first");
  const lastBtn = document.getElementById("quick-end");
  ...
  const stepTo = async (stepIndex, options = {}) => {
    ...
    const backwardAction = state.actions[clamped];
    if (options.animate && clamped === state.actionIndex + 1 && forwardAction) {
      if (forwardAction.type === "swap") {
    ...
    if (options.animate && clamped === state.actionIndex - 1 && backwardAction) {
      if (backwardAction.type === "swap") {
        await applyAction(backwardAction);
        state.actionIndex = clamped;
        setSteps(clamped, state.actions.length);
        updateStepSlider();
        renderStepList(clamped);
